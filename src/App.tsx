...
export default function App() {
  ...
  const [selectedTemplate, setSelectedTemplate] =
    useState<AdventureTemplate | null>(null);
        ...
        const acts = snapshot.docs.map((doc) => {
          ...
          return {
            ...
            availableDates: data.availableDates
              ? data.availableDates.map((d: string) => new Date(d))
              : [],
          } as AdventureTemplate;
        });
        setActivities(acts);

        const linkedActivity = new URLSearchParams(window.location.search).get(
          "activity",
        );
        if (linkedActivity) {
              ...
              docs.forEach((doc) => {
                ...
                if (template) {
                  loadedAdventures.push({
                    ...
                    date: data.date
                      ? new Date(data.date)
                      : data.createdAt?.toDate?.() || template.date,
  ...
  const accountUser =
    ...
    (() => {
      ...
      return localUser
        ? { name: localUser.displayName, contactValue: localUser.email }
        : null;
    })();
  ...
  if (currentView === "admin" && isAdministrator)
    return (
      ...
      <div className="w-full h-[100dvh] sm:h-[844px] max-w-[390px] bg-white sm:rounded-[2.5rem] sm:shadow-2xl overflow-hidden relative flex flex-col items-stretch border-slate-800 sm:border-[8px]">
        ...
        {!selectedTemplate && (
          <BottomNav
            ...
            hasAdventure={adventures.length > 0}
          />
        )}
