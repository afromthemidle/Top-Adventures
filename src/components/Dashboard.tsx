...
export function Dashboard({
  ...
  useEffect(() => {
    if (initialAdventureId && adventures.length > 0) {
      const adv =
        adventures.find((a) => a.id === initialAdventureId) ||
        adventures.find((a) => a.activityId === initialAdventureId);
      if (adv) {
  ...
  useEffect(() => {
    ...
    const selectedDateISO =
      selectedAdventure.date instanceof Date &&
      !isNaN(selectedAdventure.date.getTime())
        ? selectedAdventure.date.toISOString()
        : "";
            ...
            >
              <div className="min-w-0">
                ...
                <div className="flex items-center text-slate-500 text-sm font-medium">
                  ...
                  {adv.date instanceof Date && !isNaN(adv.date.getTime())
                ...
                <span
                  className={`mt-2 inline-flex rounded-full px-2.5 py-1 text-xs font-bold ${adv.status === "PAID" ? "bg-emerald-100 text-emerald-800" : adv.status === "REJECTED" ? "bg-rose-100 text-rose-800" : "bg-amber-100 text-amber-800"}`}
                >
                  {adv.status === "PAID"
                    ? "Reserva confirmada"
                    : adv.status === "REJECTED"
  ...
  const generateGoogleCalendarLink = (adv: Adventure) => {
    ...
    const location = encodeURIComponent(
      `${adv.meetingPoint?.name}, ${adv.meetingPoint?.address}`,
    );
      ...
      <div className="absolute top-0 left-0 right-0 h-64 bg-slate-900 z-0 overflow-hidden">
        <img
          ...
          onError={(e) => {
            ...
            e.currentTarget.src = getAdventureImage(adventure.sport);
          }}
        />
      ...
      <div className="relative z-10 px-6 pt-16">
        ...
        >
          ...
          <div className="grid grid-cols-2 gap-4 mb-6">
            <div className="bg-slate-50 rounded-2xl p-4 border border-slate-100">
              ...
              <p className="font-bold text-slate-900 text-sm">
                {adventure.date instanceof Date &&
                !isNaN(adventure.date.getTime())
                  ? new Intl.DateTimeFormat("es-ES", {
                      day: "numeric",
                      month: "short",
                    }).format(adventure.date)
                  : "Fecha pendiente"}
              </p>
            </div>
            <div className="bg-slate-50 rounded-2xl p-4 border border-slate-100">
          ...
          <div className="bg-emerald-50/50 rounded-2xl p-4 mb-6 border border-emerald-100">
            <div className="flex items-start mb-3">
              ...
              <div>
                <p className="font-bold text-slate-900 text-sm">
                  {adventure.meetingPoint?.name}
                </p>
                <p className="text-xs text-slate-500 mt-1">
                  {adventure.meetingPoint?.address}
                </p>
              </div>
            </div>
            <a
            ...
            <div className="flex flex-wrap gap-4">
              {groupParticipants.map((p, i) => (
                <div key={i} className="flex flex-col items-center">
                  <div
                    className={`w-12 h-12 rounded-full border-2 border-white flex items-center justify-center text-xl shadow-sm mb-1 ${colors[i % colors.length]}`}
                  >
          ...
          <div className="bg-white rounded-2xl p-5 shadow-sm border border-slate-100 mb-6">
            ...
            <div className="space-y-4">
              {adventure.itinerary.map((step, index) => (
                <div
                  key={index}
                  className="flex flex-col relative pl-6 border-l-2 border-emerald-100 last:border-transparent pb-4 last:pb-0"
                >
        ...
        <div className="bg-white rounded-2xl p-5 shadow-sm border border-slate-100 mb-6">
          ...
          <p className="text-slate-500 text-xs mb-4">
            (deberás llevarlo el día de la actividad)
          </p>
        ...
        <div className="bg-orange-50 rounded-2xl p-4 border border-orange-100">
          ...
          <p className="text-orange-700/80 text-xs">
            Asegúrate de ir con calzado adecuado y mantente siempre cerca del
            guía y del resto del equipo.
          </p>
        </div>
      </div>
    </div>
