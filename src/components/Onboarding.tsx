...
export function Onboarding({
  ...
  useEffect(() => {
    const unsubscribe = auth.onAuthStateChanged((user) => {
      if (user && !auth.currentUser?.isAnonymous) {
        setProfile((current) => ({
          ...current,
          name: user.displayName || current.name || "Aventurero",
          contactValue: user.email || current.contactValue || "",
          contactMethod: "email",
        }));
  ...
  const handlePayPalSuccess = async (paymentId: string) => {
    setHasPaid(true);
    const usedProfile = profile.name
      ? profile
      : JSON.parse(localStorage.getItem("pendingProfile") || "{}");
    if (auth.currentUser) {
      try {
        let receiptUrl = "";
        let receiptName = "";
        if (paymentId.startsWith("bank_transfer") && receiptFile) {
          ...
          const receiptRef = ref(
            storage,
            `payment-receipts/${auth.currentUser.uid}/${Date.now()}-${receiptFile.name.replace(/[^a-zA-Z0-9._-]/g, "_")}`,
          );
          const uploaded = await uploadBytes(receiptRef, receiptFile, {
        ...
        const reservationData = {
          ...
          status: "PENDING",
          paymentMethod: paymentId.startsWith("bank_transfer")
            ? "transfer"
            : "paypal",
  ...
  const steps = [
    {
      ...
      subtitle:
        "Conéctate con Google para guardar tu reserva y acceder al chat de grupo de forma segura.",
      icon: (
        ...
        <div className="space-y-4 w-full">
          <div className="flex bg-slate-100 p-1 rounded-xl">
            <button
              ...
              className={`flex-1 py-2 text-sm font-bold rounded-lg ${authMode === "register" ? "bg-white text-slate-800 shadow-sm" : "text-slate-500"}`}
            >
              Crear cuenta
            </button>
            <button
              ...
              className={`flex-1 py-2 text-sm font-bold rounded-lg ${authMode === "login" ? "bg-white text-slate-800 shadow-sm" : "text-slate-500"}`}
            >
              Iniciar sesión
            </button>
          </div>
          <form
            onSubmit={async (event) => {
              ...
              try {
                const result =
                  authMode === "register"
                    ? await registerLocal(authEmail, authPassword)
                    : await loginLocal(authEmail, authPassword);
                setProfile({
                  name: result.user.displayName || "Aventurero",
                  contactValue: result.user.email || authEmail.trim(),
                  contactMethod: "email",
                });
                setStep(1);
              } catch (err: any) {
                    ...
                    : code === "auth/invalid-credential" ||
                      ...
                      : code === "auth/weak-password"
                        ...
                        : code === "auth/invalid-email"
                          ...
                          : "No se pudo completar la autenticación. Verifica Firebase e inténtalo nuevamente.",
                );
              } finally {
          ...
          <div className="flex items-center gap-3 text-xs text-slate-400">
            <span className="h-px bg-slate-200 flex-1" />o
    ...
    },
    {
      ...
      subtitle:
        "Realiza el pago para confirmar tu lugar. Este pago cubre tanto la organización como la actividad en sí.",
      icon: (
        <div className="w-20 h-20 bg-emerald-100 rounded-full flex items-center justify-center mb-8">
      ...
      content: (
        <div className="space-y-6 w-full text-center">
          <div className="bg-slate-50 p-6 rounded-2xl border border-emerald-200">
            ...
            <div className="flex justify-between items-center py-2 border-b border-emerald-100">
              ...
              <span className="font-bold text-emerald-700 text-xl">
                {selectedAdventure.activityCost} USD
              </span>
            </div>
            <p className="text-xs text-slate-500 mt-3 text-left">
              * Incluye material para la actividad y guía. No incluye comida,
              transporte ni seguro de accidentes. Un solo pago, sin sorpresas.
            </p>
          </div>
          {!hasPaid ? (
                ...
                <div className="flex flex-col gap-4 pb-4">
                  ...
                  {paymentSettings.paypal && (paymentMethod === "paypal" || !isEcuador) ? (
                      ...
                      <PayPalScriptProvider options={initialOptions}>
                        <PayPalButtons
                          ...
                          createOrder={async () => {
                            ...
                            try {
                              const amountRaw = selectedAdventure.activityCost
                                ? selectedAdventure.activityCost.replace(
                                    /[^0-9.]/g,
                                    "",
                                  )
                                : "25.00";
                          ...
                          onError={(err) => {
                            console.error(err);
                            setPaymentError(
                              "Hubo un problema al procesar PayPal.",
                            );
                          }}
                        />
                    ...
                    <div className="bg-white border border-slate-200 rounded-xl p-5 text-left shadow-sm">
                      ...
                      <button
                        onClick={() =>
                          handlePayPalSuccess("bank_transfer_" + Date.now())
                        }
          ...
          <p className="text-xs text-slate-500 mt-4">
            {hasPaid
              ? paymentMethod === "transfer"
                ? "Comprobante recibido. Guardando información..."
                : "Pago confirmado. Finalizando tu reserva..."
  ...
  const completeRegistration = (currentProfile = profile) => {
    ...
    const activityLink = `${window.location.origin}/?view=dashboard&activity=${encodeURIComponent(selectedAdventure.id)}`;
    const dateLabel = selectedAdventure.date.toLocaleDateString("es-EC", {
      weekday: "long",
      day: "numeric",
      month: "long",
      year: "numeric",
    ...
    const userEmailPromise = fetch("/api/send-email", {
      ...
      body: JSON.stringify({
        ...
        subject:
          paymentMethod === "transfer"
            ? `Recibimos tu comprobante: ${selectedAdventure.sport}`
            : `Detalles de tu reserva: ${selectedAdventure.sport}`,
        html: `
    ...
    Promise.all([userEmailPromise, adminEmailPromise])
      ...
      .then((dataArray) => {
        dataArray.forEach((data, index) => {
          if (!data.success) {
            console.error(
              `Error desde API de correo (${index === 0 ? "Usuario" : "Admin"}):`,
              data.error,
            );
          }
        });
  ...
  const generateGoogleCalendarLink = () => {
    ...
    const timeMatch = selectedAdventure.time.match(
      /(\d+):(\d+)\s*(AM|PM|am|pm)?/,
    );
    if (timeMatch) {
    ...
    const text = encodeURIComponent(
      `${selectedAdventure.sport} en ${selectedAdventure.city}`,
    );
    const dates = `${formatDate(startDate)}/${formatDate(endDate)}`;
    const details = encodeURIComponent(selectedAdventure.description || "");
    const location = encodeURIComponent(
      `${selectedAdventure.meetingPointName}, ${selectedAdventure.meetingPointAddress}`,
    );
          ...
          {paymentMethod === "transfer" ? (
            <>
              <h2 className="text-2xl font-extrabold text-slate-900 mb-4 tracking-tight">
                ¡Comprobante Recibido, {profile.name}!
              </h2>
              <div className="bg-amber-50 border border-amber-200 rounded-xl p-5 mb-6 text-amber-800 text-sm font-medium">
                Tu reserva está pendiente de verificación de pago. Lo
                verificaremos en las próximas 3 horas (horario laborable) y
                confirmaremos tu cupo.
              </div>
            </>
          ) : (
            <>
              <h2 className="text-3xl font-extrabold text-slate-900 mb-4 tracking-tight">
                ¡Todo listo, {profile.name}!
              </h2>
            ...
            <div className="flex items-center">
              ...
              <div>
                <p className="text-slate-900 font-bold text-sm capitalize">
                  {selectedAdventure.date.toLocaleDateString("es-ES", {
                    weekday: "short",
                    day: "numeric",
                    month: "short",
                  })}
      ...
      <div className="w-full shrink-0 z-10 pt-6">
        ...
        >
          {isSimulatingSending ? (
            ...
            <>
              <span className="mr-2">
                {step === steps.length - 1 ? "Confirmar Reserva" : "Continuar"}
              </span>
