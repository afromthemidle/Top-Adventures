...
async function sendEmail(request: Request, env: WorkerEnv) {
  try {
    const { to, subject, html, attachments } = await request.json() as { to?: string; subject?: string; html?: string; attachments?: Array<{ filename: string; path?: string; content?: string }> };
